...
const proof = '/tmp/real-multiboot-v9-proof';
...
page.on('requestfailed', request => {
  ...
  console.error('[requestfailed]', line);
});

function fatalLog(log) {
...
try {
  for (const width of [320, 375, 768, 1024, 1440]) {
    ...
    await page.goto(`${target}&layout=${width}`, {
    ...
    if (!layout.pass) throw new Error(`responsive layout failed at ${width}px: ${JSON.stringify(layout)}`);
  }

  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto(`${target}&boot=1`, {
    waitUntil: 'domcontentloaded',
    timeout: 120000,
  });
  await page.waitForSelector('#boot-btn:not([disabled])');
  ...
  while (Date.now() < deadline) {
    ...
    result.completion_logged ||= log.includes('display completion trigger=')
      && log.includes('display ready preset=gorics');
    ...
    if (fatalLog(log)) throw new Error('fatal marker found in browser log');

    const elapsed = Date.now() - startedAt;
    if (elapsed - lastProgressScreenshot >= 120000) {
      lastProgressScreenshot = elapsed;
      await page.screenshot({
        path: `${proof}/progress-${Math.floor(elapsed / 1000)}s.png`,
        fullPage: true,
      });
    }
  }
  ...
  if (!result.success) {
    throw new Error(`unobscured desktop timeout: ${JSON.stringify(result.ui)}`);
  }
  ...
  result.stable_after_5s = stable;
  if (!(stable.hidden
      && stable.aria_hidden
      && stable.visibility === 'hidden'
      && stable.opacity <= 0.01
      && stable.progress === '100%'
      && stable.boot_text === '실행 중')) {
    throw new Error(`completion state was not stable: ${JSON.stringify(stable)}`);
  }

  await page.screenshot({ path: `${proof}/99-desktop-unobscured.png`, fullPage: true });
  const canvas = page.locator('#screen canvas');
  if (await canvas.count()) {
    await canvas.screenshot({ path: `${proof}/99-screen-canvas.png` });
  }
} catch (error) {
