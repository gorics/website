(function () {
  if (window.__globalEnhancementsInit) return;
  window.__globalEnhancementsInit = true;
  ...
  const init = () => {
    ...
    const rawAttr = attrSource(body, 'data-arcade-game') || attrSource(html, 'data-arcade-game');
    ...
    const wantsMobileControls =
      ...
      (controlsPref === 'true' ||
        controlsPref === 'on' ||
        attr === 'true' ||
        (!attr && !controlsPref && looksLikeGameRoute));

    if (wantsMobileControls && !attr) {
      body.setAttribute('data-arcade-game', 'true');
      if (html) html.setAttribute('data-arcade-game', 'true');
    }

    if (wantsMobileControls && !controlsPref) {
      body.setAttribute('data-mobile-controls', 'on');
      if (html) html.setAttribute('data-mobile-controls', 'on');
    }
  ...
  `;
    ...
    const wrappers = Array.from(
      doc.querySelectorAll(
        '.panel, .container, .wrapper, .frame, main, .game-shell, .hud, .board-shell, .playfield, .game-card'
      )
    );
