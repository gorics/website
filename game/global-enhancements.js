(function () {
  ...
  const inferGameContext = () => {
    if (!body) return false;

    if (body.hasAttribute('data-arcade-game')) {
      const attr = body.getAttribute('data-arcade-game');
      return attr !== 'false';
    }
    ...
    const next = segments[gameIndex + 1];
    if (!next) return false;
    if (next === 'index.html') return false;

    return true;
  };

  const wantsMobileControls = inferGameContext();
